...
import { fetchContactDetail, type ApiContactDetail } from "@/lib/api";
import {
  BlocklistedBadge,
  ResponseBadge,
  StatusBadge,
...
/** Large right-side modal with a contact's full profile, per-webinar history
 *  (list, status, calendar response) and attributed bookings. */
export function ContactDetailDrawer({
  ...
  return (
    <div className="fixed inset-0 z-[60]">
      {/* Overlay */}
      <div
        className="absolute inset-0 bg-zinc-950/40 dark:bg-zinc-950/60 backdrop-blur-[2px] animate-in fade-in duration-150"
        onClick={onClose}
      />

      {/* Panel */}
      <div className="absolute inset-y-0 right-0 w-full max-w-[680px] bg-white dark:bg-zinc-900 border-l border-zinc-200 dark:border-zinc-800/60 shadow-2xl flex flex-col animate-in slide-in-from-right-8 duration-200">
            ...
            <div className="space-y-6">
              ...
              >
                {detail.webinar_history.length === 0 ? (
                  ...
                  <div className="rounded-lg border border-zinc-200 dark:border-zinc-800/40 overflow-hidden">
                          ...
                          <tr key={h.webinar_id} className="border-b border-zinc-100 dark:border-zinc-800/30 last:border-b-0 align-top">
                            ...
                            <td className="px-3 py-2.5">
                              {h.membership_status ? (
                                ...
                                <span className="text-[11px] text-zinc-400" title="No current membership — e.g. released after the invite, or matched from calendar data only">
                                  no longer on list
                                </span>
                              )}
                            </td>
                            <td className="px-3 py-2.5">
                              ...
                              {h.calendar_account && (
                                <div className="text-[11px] text-zinc-400 truncate max-w-[160px]" title={h.calendar_account}>
                                  {h.calendar_account}
                                </div>
                              )}
                            </td>
...
function FieldGrid({ children }: { children: React.ReactNode }) {
  return <dl className="grid grid-cols-2 gap-x-6 gap-y-2.5">{children}</dl>;
}
...
function Th({ children }: { children: React.ReactNode }) {
  return (
    <th className="text-left px-3 py-2 text-[11px] text-zinc-500 uppercase tracking-wider font-medium">
      {children}
    </th>
  );
}
