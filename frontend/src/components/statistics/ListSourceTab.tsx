...
// Frozen top-left corner: sticks to both top and left so the source label stays
// visible while scrolling right. Higher z than the row-1/row-2 header cells.
...
function GroupedFunnelTable({
  ...
  // Frozen label-column header cell — sticks left AND top-6, above the scrolling
  // header cells (z-30) so the source/vintage label header stays put on scroll.
  const headCorner =
  ...
  return (
    <>
      <tr className="bg-white dark:bg-zinc-950 hover:bg-zinc-50 dark:hover:bg-zinc-900/60">
        <td className="sticky left-0 z-20 bg-white dark:bg-zinc-950 px-3 py-2 text-left text-zinc-800 dark:text-zinc-200 font-medium">
          <button
            onClick={onToggle}
            disabled={!canExpand}
        ...
        row.children.map((child) => {
          ...
          return (
            <tr key={child.key} className="bg-zinc-50/60 dark:bg-zinc-900/40">
              <td className="sticky left-0 z-20 bg-zinc-50 dark:bg-zinc-900 px-3 py-1.5 pl-9 text-left text-zinc-500 dark:text-zinc-400 text-[11px]" title={childLabel}>
